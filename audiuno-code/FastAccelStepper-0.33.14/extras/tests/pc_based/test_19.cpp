 ...
 public:
  ...
  void ramp(uint32_t accel, uint32_t speed_us, uint32_t steps,
    ...
    for (int i = 0; i < 100 * steps; i++) {
      ...
      if (s.getPositionAfterCommandsCompleted() >= steps) {
        break;
      }
...
int main() {
  ...
  // Test related to #347 to reach max speed
  //     accel   speed_us (coast)   steps   reach_coasting
  test.ramp(100, 5, 51000000, false);
